...
export default class AutoLanguageClient {
  ...
  _busySignalService: atomIde$BusySignalService;
  _busyMessage: ?atomIde$BusyMessage;
  ...
  activate(): void {
    ...
    this._serverManager = new ServerManager(
      ...
      title => this.setBusyMessage(title, false, true),
    );
  ...
  async startServer(projectPath: string): Promise<ActiveServer> {
    ...
    this.setBusyMessage(`initializing for ${projectPath}`, false, true);
  ...
  consumeBusySignal(busySignalService: atomIde$BusySignalService): void {
    this._busySignalService = busySignalService;
  }

  setBusyMessage(title?: string, revealTooltip?: boolean = false, prefixWithName?: boolean = true): void {
    const completeTitle = prefixWithName ? `${this.name} ${title}` : title;

    if (this._busyMessage != null && (title == null || revealTooltip)) {
    ...
    if (title != null) {
      if (this._busyMessage != null && !revealTooltip) {
        this._busyMessage.setTitle(completeTitle);
        return;
      }

      if (this._busySignalService != null) {
        this._busyMessage = this._busySignalService.reportBusy(completeTitle, {revealTooltip});
      }
    }
  }
}
