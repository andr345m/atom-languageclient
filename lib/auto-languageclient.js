...
export default class AutoLanguageClient {
  ...
  async startServer(projectPath: string): Promise<ActiveServer> {
    this.setBusyMessage(`Starting ${this.getLanguageName()} language support`);
    ...
    this.setBusyMessage(`Initializing ${this.getLanguageName()} language support`);
  ...
  setBusyMessage(title?: string, revealTooltip?: boolean = false): void {
    ...
    if (title != null) {
      if (this._busyMessage != null && !revealTooltip) {
        this._busyMessage.setTitle(title);
        return;
      }

      if (this._busySignalService != null) {
        this._busyMessage = this._busySignalService.reportBusy(title, { revealTooltip });
      }
    }
  }
}
